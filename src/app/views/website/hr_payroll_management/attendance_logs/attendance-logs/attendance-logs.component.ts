...
export class AttendanceLogsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getViewStatus();
    this.access.refresh();
    this.showActionColumn = this.access.canEdit(this.featureRef) || this.access.canApprove(this.featureRef) ;
    this.baseHeaders = [
      'Employee Name',
      'Location',
      'Date',
  ...
  notAccess(){
    this.uiUtils.showWarningToster("You don't have access this feature")
  }
  ...
  get headers(): string[] {
    ...
    return [...this.baseHeaders, 'Leave Type', 'Status', ...(this.showActionColumn ? ['Action'] : []),];
  }
