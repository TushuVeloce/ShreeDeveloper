...
export class BillsPayableComponent implements OnInit {
  ...
  featureRef: ApplicationFeatures = ApplicationFeatures.BillPayableReport;
  ...
  get totalInvoiceAmount(): number {
    return this.DisplayMasterList.reduce(
      (sum, item) => sum + (item.p.InvoiceAmount || 0),
      0
    );
  }
  ...
  get totalRemainingAmount(): number {
    return this.DisplayMasterList.reduce(
      (sum, item) => sum + (item.p.RemainingAmount || 0),
      0
    );
  }
  ...
  get totalCashReceived(): number {
    return this.DisplayMasterList.reduce(
      (sum, item) => sum + (item.p.TotalCashReceived || 0),
      0
    );
  }
  ...
  get totalChequeReceived(): number {
    return this.DisplayMasterList.reduce(
      (sum, item) => sum + (item.p.TotalChequeReceived || 0),
      0
    );
  }
