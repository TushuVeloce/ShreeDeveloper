...
export class CustomerFollowUp implements IPersistable<CustomerFollowUp> {
  ...
  public CheckSaveValidity(
    ...
    if (this.p.CustomerRequirement == '')
      vra.add('CustomerRequirement', 'CustomerRequirement cannot be blank.');
    if (this.p.LeadSource == 0)
      vra.add('LeadSource ', 'LeadSource  cannot be blank.');
    if (this.p.LeadHandleBy == 0)
      vra.add('LeadHandelBy', 'LeadHandelBy  cannot be blank.');
    // if (this.p.BrokerName == '') vra.add('BrokerName', 'BrokerName cannot be blank.');
    if (this.p.ContactMode == 0)
    ...
    if (this.p.IsNewlyCreated) {
      if (
        this.p.CustomerStatus !== CustomerStatus.LeadClosed &&
        this.p.CustomerStatus !== CustomerStatus.LeadClosed &&
        !this.p.ReminderDate
      ) {
        vra.add(
  ...
  public MergeIntoTransportData(td: TransportData) {
    DataContainerService.GetInstance().MergeIntoContainer(
      td.MainData,
      CustomerFollowUp.Db_Table_Name,
      this.p
    );
  }

  private static m_currentInstance: CustomerFollowUp =
  ...
  public static async FetchTransportData(
    req: CustomerFollowUpFetchRequest,
  ...
  public static async FetchEntireListByCustomerFollowUpRef(
    CustomerFollowUpRef: number,
    errorHandler: (err: string) => Promise<void> = UIUtils.GetInstance()
      .GlobalUIErrorHandler
  ) {
    let req = new CustomerFollowUpFetchRequest();
  ...
  public static async FetchEntireListByPlotRef(
    plotref: number,
    errorHandler: (err: string) => Promise<void> = UIUtils.GetInstance()
      .GlobalUIErrorHandler
  ) {
  ...
  public static async FetchEntireListByDate(
    ReminderDate: string,
    errorHandler: (err: string) => Promise<void> = UIUtils.GetInstance()
      .GlobalUIErrorHandler
  ) {
  ...
  public static async FetchEntireListBySiteRef(
    siteref: number,
    errorHandler: (err: string) => Promise<void> = UIUtils.GetInstance()
      .GlobalUIErrorHandler
  ) {
  ...
  public static async FetchEntireListByDateandPlotRef(
    ReminderDate: string,
    plotref: number,
    errorHandler: (err: string) => Promise<void> = UIUtils.GetInstance()
      .GlobalUIErrorHandler
